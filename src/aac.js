...
let MPEG_IDENTIFIER_MPEG2 = 1;
let MPEG_IDENTIFIER_MPEG4 = 0;
...
var api = {
  ...
  getNextPossibleSyncwordPosition(buffer) {
    let syncwordPos = Bits.searchBitsInArray(buffer, [0xff, 0xf0], 1);
  ...
  skipToNextPossibleSyncword() {
    let syncwordPos = Bits.searchBitsInArray(audioBuf, [0xff, 0xf0], 1);
    if (syncwordPos > 0) {
      // The maximum distance between two syncwords is 8192 bytes.
      if (syncwordPos > 8192) {
        throw new Error(`the next syncword is too far: ${syncwordPos} bytes`);
  ...
  splitIntoADTSFrames(buffer) {
    ...
    while (true) {
      ...
      let aac_frame_length = Bits.parse_bits_uint(buffer, 30, 13);
      ...
      let adtsFrame = buffer.slice(0, aac_frame_length);
  ...
  feedPESPacket(pesPacket) {
    ...
    while (true) {
      ...
      let aac_frame_length = Bits.parse_bits_uint(audioBuf, 30, 13);
      ...
      let adtsFrame = audioBuf.slice(0, aac_frame_length);
  ...
  parseAudioSpecificConfig(buf) {
    let bits = new Bits(buf);
    let asc = api.readAudioSpecificConfig(bits);
    return asc;
  },
    ...
    if (extensionIdentifier === 0x76a) {
      ...
      if ((info.audioObjectType !== 30) && (bits.get_remaining_bits() >= 1)) {
        ...
        if (info.mpsPresentFlag === 1) {
          ...
          if (info.sscLen === 0xff) {
            let sscLenExt = bits.read_bits(16);
  ...
  parseADTSFrame(adtsFrame) {
    ...
    let profile_ObjectType = Bits.parse_bits_uint(adtsFrame, 16, 2);
    ...
    let freq = Bits.parse_bits_uint(adtsFrame, 18, 4);
